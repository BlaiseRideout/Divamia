...
void Demo::initTexture() {
  texture = ShaderProgram({
    FragmentShader("res/texture.frag"),
    VertexShader("res/texture.vert")
  });

  Texture tex("./res/fish_sandwich_small.png");
  texture["tex"] = tex;
  texture["P"] = glm::ortho(0.0f, static_cast<float>(window.width), static_cast<float>(window.height), 0.0f);
  texture["V"] = glm::mat4(1.0f);
  static constexpr float displayWidth = 200.0f;
  texture["M"] = glm::translate(glm::scale(glm::mat4(1.0f), glm::vec3(displayWidth, displayWidth * tex.height() / tex.width(), 0.0f)), glm::vec3(1.5f, 1.5f, 0.0f));
...
void Demo::initTriangle() {
  triangle = ShaderProgram({
    FragmentShader("res/test.frag"),
    VertexShader("res/test.vert")
  });

  triangle["P"] = glm::ortho(0.0f, static_cast<float>(window.width), static_cast<float>(window.height), 0.0f);
  triangle["V"] = glm::mat4(1.0f);
  triangle["M"] = glm::translate(glm::scale(glm::mat4(1.0f), glm::vec3(200.0f)), glm::vec3(0.5f, 0.5f, 0.0f));
