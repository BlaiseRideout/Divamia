...
Texture::Texture(const char *filename, GLint mag_filter, GLint min_filter) :
    Texture(Image(filename), mag_filter, min_filter) {}

Texture::Texture(const std::string &filename, GLint filter) : Texture(filename.c_str(), filter) {}

Texture::Texture(const std::string &filename, GLint mag_filter, GLint min_filter) :
    Texture(filename.c_str(), mag_filter, min_filter) {}
...
Texture::Texture(Image &img, GLint mag_filter, GLint min_filter) :
...
Texture::Texture(Image &&img, GLint mag_filter, GLint min_filter) :
    Texture(img, mag_filter, min_filter) {}

Texture::Texture(unsigned width, unsigned height, GLint mag_filter, GLint min_filter) :
    mag_filter(mag_filter),
    min_filter(min_filter) {
  glGenTextures(1, &this->id);
  incRef(this->id);
