...
static std::unordered_map<FREE_IMAGE_FORMAT, const char *> formatStrings{
    {FIF_UNKNOWN, "FIF_UNKNOWN"}, {FIF_BMP, "FIF_BMP"},       {FIF_ICO, "FIF_ICO"},
    {FIF_JPEG, "FIF_JPEG"},       {FIF_JNG, "FIF_JNG"},       {FIF_KOALA, "FIF_KOALA"},
    {FIF_LBM, "FIF_LBM"},         {FIF_IFF, "FIF_IFF"},       {FIF_MNG, "FIF_MNG"},
    {FIF_PBM, "FIF_PBM"},         {FIF_PBMRAW, "FIF_PBMRAW"}, {FIF_PCD, "FIF_PCD"},
    {FIF_PCX, "FIF_PCX"},         {FIF_PGM, "FIF_PGM"},       {FIF_PGMRAW, "FIF_PGMRAW"},
    {FIF_PNG, "FIF_PNG"},         {FIF_PPM, "FIF_PPM"},       {FIF_PPMRAW, "FIF_PPMRAW"},
    {FIF_RAS, "FIF_RAS"},         {FIF_TARGA, "FIF_TARGA"},   {FIF_TIFF, "FIF_TIFF"},
    {FIF_WBMP, "FIF_WBMP"},       {FIF_PSD, "FIF_PSD"},       {FIF_CUT, "FIF_CUT"},
    {FIF_XBM, "FIF_XBM"},         {FIF_XPM, "FIF_XPM"},       {FIF_DDS, "FIF_DDS"},
    {FIF_GIF, "FIF_GIF"},         {FIF_HDR, "FIF_HDR"},       {FIF_FAXG3, "FIF_FAXG3"},
    {FIF_SGI, "FIF_SGI"},         {FIF_EXR, "FIF_EXR"},       {FIF_J2K, "FIF_J2K"},
    {FIF_JP2, "FIF_JP2"},         {FIF_PFM, "FIF_PFM"},       {FIF_PICT, "FIF_PICT"},
    {FIF_RAW, "FIF_RAW"},
};
...
Image::Image(unsigned width, unsigned height, unsigned char *temppix, unsigned bpp) :
    Image(width,
          height,
          std::vector<unsigned char>(temppix, temppix + width * height * bpp / BITS_PER_BYTE)) {}

Image::Image(unsigned width, unsigned height, unsigned bpp) :
    Image(width, height, std::vector<unsigned char>(width * height * bpp / BITS_PER_BYTE)) {}

Image::~Image() {
  if(_freeImage != nullptr) FreeImage_Unload(_freeImage);
  _freeImage = nullptr;
}
...
void Image::loadData(unsigned width, unsigned height, std::vector<unsigned char> pix) {
  ...
  _freeImage = FreeImage_ConvertFromRawBits(_pix.data(),
                                            _width,
                                            _height,
                                            _width,
                                            _bpp,
