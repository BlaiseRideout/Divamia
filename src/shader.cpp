...
void Shader::del() {
  ...
  if(iterid != Shader::refCount.end()) Shader::refCount.erase(iterid);
}
...
GeometryShader::GeometryShader(std::string const &filename) :
...
TessControlShader::TessControlShader(std::string const &filename) :
    Shader(filename, GL_TESS_CONTROL_SHADER) {}
TessEvaluationShader::TessEvaluationShader(std::string const &filename) :
...
void ShaderProgram::del() {
  ...
  if(uid != ShaderProgram::_uids.end()) ShaderProgram::_uids.erase(uid);

  auto aid = ShaderProgram::_aids.find(this->id);
  if(aid != ShaderProgram::_aids.end()) ShaderProgram::_aids.erase(aid);

  auto tex = ShaderProgram::gtextures.find(this->id);
  if(tex != ShaderProgram::gtextures.end()) ShaderProgram::gtextures.erase(tex);

  auto iterid = ShaderProgram::refCount.find(this->id);
  if(iterid != ShaderProgram::refCount.end()) ShaderProgram::refCount.erase(iterid);
}
...
std::vector<Texture> &ShaderProgram::textures() const {
  if(this->id == 0) throw std::runtime_error("Error: Shader program not loaded yet");
...
std::map<std::string, GLuint> &ShaderProgram::uids() const {
  ...
  if(i == _uids.end())
    i = _uids.insert(_uids.end(),
                     std::pair<GLuint, std::map<std::string, GLuint>>(
                         this->id, std::map<std::string, GLuint>()));
