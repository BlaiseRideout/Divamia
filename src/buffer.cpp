...
Buffer::Buffer(Buffer const &b) : id(b.id), size(b.size), target(b.target), _elemSize(b._elemSize) {
  ...
  if(iterid != Buffer::refCount.end()) iterid->second = iterid->second + 1;
}
...
Buffer &Buffer::operator=(Buffer const &b) {
  ...
  if(iterid != Buffer::refCount.end()) {
    iterid->second = iterid->second - 1;
    if(iterid->second == 0) glDeleteBuffers(1, &this->id);
  }
  this->id = b.id;
  this->size = b.size;
...
void Buffer::setAttrib(const ShaderProgram &s,
...
void Buffer::setAttrib(
    const ShaderProgram &s, const char *name, bool normalized, int divisor, GLenum type) const {
...
void Buffer::drawInstanced(GLenum mode, int count, GLenum type) const {
  this->bind(GL_ELEMENT_ARRAY_BUFFER);
  glDrawElementsInstanced(mode, this->size, type, (void *)0, count);
}
...
IndexBuffer::IndexBuffer(std::vector<unsigned> const &data, GLenum usage) :
    Buffer(data, GL_ELEMENT_ARRAY_BUFFER, usage) {}

IndexBuffer::IndexBuffer(std::vector<unsigned> const &data) :
    Buffer(data, GL_ELEMENT_ARRAY_BUFFER) {}
